...
- 객체들의 필드/게터/세터 들을 직접 접근하는 것이 해당 객체의 캡슐화를 위반하는 것 일때 사용
  - 객체가 스스로 자신의 상태를 스냅의 생성을 담당하게 해서 원래 객체의 상태 데이터는 안전함
...
- 프로토 타입이 메멘토 패턴의 더 간단한 대한이 될 수 있으며, 상태를 기록에 저장하려는 객체가 간단하고 외부 리소스에 대한 링크가 없거나 링크가 있어도 이들을 재설정하기 쉬운 경우에 작동